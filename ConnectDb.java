 ...
 */
   ...
   public Boolean UpdateDocumentation(String id,String Text) throws SQLException,ClassNotFoundException {
     ...
     public Boolean CreateDocumentation(String EntId,String Text) throws SQLException,ClassNotFoundException {
            try ( 
                Connection connection = DriverManager.getConnection(this.jdbcURL, this.dbUser, this.dbPassword)) {
                String sql = "INSERT INTO `documentation` (id,`Details`, `EntId`) VALUES (1,?, ?)";
                PreparedStatement statement = connection.prepareStatement(sql);
...
public boolean DeriverPackage() throws SQLException,ClassNotFoundException {
    ...
    for(int i=0;i<data.size();i++){
        ...
        try (Connection connection = DriverManager.getConnection(this.jdbcURL, this.dbUser, this.dbPassword)) {
                String sql = "INSERT INTO `inventrory` ( `name`, `EnterpriceId`, `Cost`, `Amount`) VALUES (?, ?, ?, ?)";
                PreparedStatement statement = connection.prepareStatement(sql);
                
                statement.setString(1, data.get(i).GetName());
                statement.setString(2, data.get(i).GetName());
   ...
   Vector<JabDetails> jab =new Vector<JabDetails>();
       jab.clear();
            try ( 
                ...
                ResultSet result = statement.executeQuery(sql);
                if(result.next()) { 
                    jab.add(new JabDetails(result.getString("id"), result.getString("Amount"), result.getString("Cost"), result.getString("Time"),result.getString("EnterpriceId"))); 
                }
                connection.close();
                }  
   ...
   Vector<NoDocument>doc =new Vector<NoDocument>();
       doc.clear();
            try ( 
                ...
                ResultSet result = statement.executeQuery(sql);
                if(result.next()) { 
                    doc.add(new JabDetails(result.getString("id"), result.getString("NationalId"), result.getString("lname"), result.getString("Date"),result.getString("lname"))); 
                }
                connection.close();
                }  
